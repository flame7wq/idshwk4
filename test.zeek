...
event http_reply(c: connection, version: string, code: count, reason: string) 
{
    ...
    if (code == 404) 
    {
        SumStats::observe("response404", 
                           SumStats::Key($host=c$id$orig_h), 
                           SumStats::Observation($num=1));
        SumStats::observe("responseUnique404", 
                           SumStats::Key($host=c$id$orig_h), 
...
event zeek_init() 
{
    local rAll = SumStats::Reducer($stream="response", 
                                   $apply=set(SumStats::SUM));
    local r404 = SumStats::Reducer($stream="response404", 
                                   $apply=set(SumStats::SUM));
    local rUnique404 = SumStats::Reducer($stream="responseUnique404", 
                                         $apply=set(SumStats::UNIQUE));

    SumStats::create([$name="idshwk4", 
                      $epoch=10min, 
                      $reducers=set(rAll, r404, rUnique404), 
